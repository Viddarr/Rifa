...
CREATE TABLE IF NOT EXISTS rifas (
  ...
  meta_bilhetes  INTEGER,                  -- NULL = sem limite
  meta_valor     NUMERIC(10,2),            -- NULL = sem meta de valor
  data_sorteio   DATE,
  status         TEXT        NOT NULL DEFAULT 'ativa'
                             CHECK (status IN ('ativa','encerrada','sorteada','cancelada')),
  ganhador_id    INTEGER     REFERENCES participantes(id),
...
-- ──────────────────────────────────────────────────────────────────────
-- BILHETES
-- Criados só após pagamento confirmado
-- ──────────────────────────────────────────────────────────────────────
...
CREATE TABLE IF NOT EXISTS webhooks_log (
  id          SERIAL      PRIMARY KEY,
  txid        TEXT,
  payload     JSONB,                       -- PostgreSQL suporta JSONB nativo
...
-- ══════════════════════════════════════════════════════════════════════
-- ADMIN INICIAL
-- Senha padrão: admin123  — TROQUE antes de ir pra produção
