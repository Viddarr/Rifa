...
router.post('/', async (req, res) => {
  try {
    ...
    const resultado = await transaction(async (client) => {
      // 1. Reaproveita o participante se o CPF já existir, senão cria um novo
      //    (evita violar a constraint UNIQUE(cpf) em compras repetidas da mesma pessoa)
      const { rows: [participanteExistente] } = await client.query(
      ...
      if (participanteExistente) {
        const { rows: [atualizado] } = await client.query(
          'UPDATE participantes SET nome = $1, telefone = $2 WHERE id = $3 RETURNING *',
          [nome.trim(), telefone.trim(), participanteExistente.id]
        );
